function [oae,tim,x,RR,UU,UUr,oaeSt,TMdisp,UUr1]=CochleaRGB_Tone_AVDisertace_Tone2BF2(A1o,F1o,A2o,F2o,gain,phi1o,phi2o,T12ono,visual,TMres,faca0,facb0,OPo)
...
%        UUr(:,count+1)= undamp(ind1:ind2).*((nonlin(y(2*N+ind1:2*N+ind2)))-y(2*N+ind1:2*N+ind2));%U?
       UU(:,count+1)= undamp(ind1:ind2).*(nonlin(y(2*N+ind1:2*N+ind2)+Opsh)-nonlin(Opsh));%U?
       UUr(:,count+1)= undamp(ind1:ind2).*((nonlin(y(2*N+ind1:2*N+ind2)+Opsh)-nonlin(Opsh))-y(2*N+ind1:2*N+ind2));%U?qq1d
