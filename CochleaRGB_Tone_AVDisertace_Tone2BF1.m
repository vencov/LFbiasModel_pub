function [oae,tim,x,RR,UU,UUr,oaeSt,TMdisp,UUr1]=CochleaRGB_Tone_AVDisertace_Tone2BF1(A1o,F1o,A2o,F2o,gain,phi1o,phi2o,T12ono,visual,TMres,faca0,facb0,OPo)
...
if nargin < 11, faca0=0.1; end;  %parameter of TM transfer function
if nargin < 12, facb0=0.1; end;  %parameter TM transfer function
...
global A1 A2 F1 F2 phi1 phi2 T12on  faca facb Opsh

ResetAll; %reset global

A1=A1o;A2=A2o;F1=F1o;F2=F2o;phi1=phi1o; phi2=phi2o;Opsh = OPo; 
...
for k=1:length(t1)
    [Sig(k)]=Inp_sig2PrimariesME(F1,A1,F2,A2,t1(k),phi1,phi2,T12on);
end
...
%        UUr(:,count+1)= undamp(ind1:ind2).*((nonlin(y(2*N+ind1:2*N+ind2)))-y(2*N+ind1:2*N+ind2));%U?
       UU(:,count+1)= undamp(ind1:ind2).*(nonlinSy(y(2*N+ind1:2*N+ind2)+Opsh)-nonlinSy(Opsh));%U?
       UUr(:,count+1)= undamp(ind1:ind2).*((nonlinSy(y(2*N+ind1:2*N+ind2)+Opsh)-nonlinSy(Opsh))-y(2*N+ind1:2*N+ind2));%U?qq1d
       UUr1(:,count+1)= undamp(ind1:ind2).*((nonlinSy(y(2*N+ind1:2*N+ind2)+Opsh)-nonlinSy(Opsh))-qq1d*y(2*N+ind1:2*N+ind2));
...
function ResetAll
 ...
 global  Sf A1 A2 F1 F2 phi1 phi2 T12on 
...
global A1 A2 F1 F2 phi1 phi2 T12on faca facb Opsh
    ...
    inp=Inp_sig2PrimariesME(F1,A1,F2,A2,t,phi1,phi2,T12on);
    ...
    Ycut=nonlinSy(Y+Opsh) - nonlinSy(Opsh);   
    dV = -1*Ginv*(stiff.*X + DampSp*V + undamp.*Ycut - Gs*MEinv*hME*Vme - Gs*MEinv*kME*Xme + Gs*MEinv*Sow*Gme*inp);
    dVme = -1*MEinv*(hME*Vme + kME*Xme + Sow*Qbm*dV/Wbm - Sow*Gme*inp);
